class AABB {
    ...
    applyImoulse(impulseX: number, impulseY: number) {
        this.velocityX += impulseX / this.mass;
        this.velocityY += impulseY / this.mass;
    }
    ...
    checkCollision(collisionObject: AABB): { penetrationComponent: "x" | "y", positionChange:/*on penetration axis*/number } | null {
        ...
        if (minMaxA.minX <= minMaxB.maxX &&
            ...
            const dx = minMaxB.minX > minMaxA.minX ? minMaxA.maxX - minMaxB.minX : minMaxB.maxX - minMaxA.minX,
    ...
    pointIsInside(pointX: number, pointY: number): boolean {
        return (
            pointX >= this.x &&
    ...
    getMinMaxStuff(): { minX: number, minY: number, maxX: number, maxY: number } {
        return {
            minX: this.x,
            minY: this.y,
